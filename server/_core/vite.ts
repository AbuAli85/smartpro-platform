...
export function serveStatic(app: Express) {
  ...
  app.use("*", (req, res) => {
    ...
    if (req.originalUrl.startsWith("/api/")) {
      // If we reach here for an API route, something went wrong
      // Log it but don't send HTML - return JSON error instead
      console.error(`[Static Catch-All] CRITICAL: API route reached static catch-all: ${req.originalUrl}`, {
        headersSent: res.headersSent,
        method: req.method,
      });
      // If response wasn't sent, return JSON error (not HTML!)
      if (!res.headersSent) {
